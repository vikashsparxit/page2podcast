...
model: opus
---
Read the visual-explainer skill at .claude/skills/visual-explainer/SKILL.md and follow its workflow, then generate a comprehensive visual plan review as a self-contained HTML page, comparing the current codebase against a proposed implementation plan.
...
7. **Risk assessment** — styled cards for:
   ...
   - **Cognitive complexity** — areas where the plan introduces non-obvious coupling, action-at-a-distance behavior, implicit ordering requirements, or contracts that exist only in the developer's memory. Each cognitive complexity flag gets a brief mitigation suggestion.
